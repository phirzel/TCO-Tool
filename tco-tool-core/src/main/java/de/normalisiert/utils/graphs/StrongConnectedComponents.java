...
/**
 * This is a helpclass for the search of all elementary cycles in a graph with the algorithm of Johnson. For this it searches for strong connected components, using the algorithm of Tarjan. The
 * constructor gets an adjacency-list of a graph. Based on this graph, it gets a nodenumber s, for which it calculates the subgraph, containing all nodes {s, s + 1, ..., n}, where n is the highest
 * nodenumber in the original graph (e.g. it builds a subgraph with all nodes with higher or same nodenumbers like the given node s). It returns the strong connected component of this subgraph which
 * contains the lowest nodenumber of all nodes in the subgraph.<br><br>
 * <p>
 * For a description of the algorithm for calculating the strong connected components see:<br> Robert Tarjan: Depth-first search and linear graph algorithms. In: SIAM Journal on Computing. Volume 1,
 * Nr. 2 (1972), pp. 146-160.<br> For a description of the algorithm for searching all elementary cycles in a directed graph see:<br> Donald B. Johnson: Finding All the Elementary Circuits of a
...
public class StrongConnectedComponents {
    ...
    /**
     * This method returns the adjacency-structure of the strong connected component with the least vertex in a subgraph of the original graph induced by the nodes {s, s + 1, ..., n}, where s is a
     * given node. Note that trivial strong connected components with just one node will not be returned.
     *
     * @param node node s
     * @return SCCResult with adjacency-structure of the strong connected component; null, if no such component exists
     */
    ...
    /**
     * Calculates the strong connected component out of a set of scc's, that contains the node with the lowest index.
     *
    ...
    /**
     * @return Vector[]::Integer representing the adjacency-structure of the strong connected component with least vertex in the currently viewed subgraph
     */
