...
class ReportDepreciationCostBlock extends ReportTool {
	...
	protected ReportDepreciationCostBlock(ModelUtility utility, final String title) {
		super(utility, title + " [" + ResourceManager.getResource(SEPlugin.class, "CTCompoundInterest") + "]");
	}

	/**
	 * Summarize all Depreciation-Cost within given object and sub-packages.
	 */
	public static ReportTool createBlockDepreciation(ModelUtility utility, TcoObject root, long maxDepreciation, ClassLoader loader) throws java.lang.Exception {
		ReportDepreciationCostBlock tool = new ReportDepreciationCostBlock(utility, ResourceManager.getResource(SEPlugin.class,
			"MniReportFinancialCostBlockTotal_text", loader));
		tool.loader = loader;
	 ...
	 */
	public static ReportTool createBlockDetailedDepreciation(ModelUtility utility, TcoObject root, long maxDepreciation, ClassLoader loader)
		throws java.lang.Exception {
		ReportDepreciationCostBlock tool = new ReportDepreciationCostBlock(utility, ResourceManager.getResource(SEPlugin.class,
			"MniReportFinancialCostBlockDetailed_text", loader));
		tool.loader = loader;
		tool.calculator = new CalculatorLinearDepreciationCompountInterest(LauncherView.getInstance().getUtility(), root, maxDepreciation);
		if (tool.calculator.isBaseOffsetInvolved()) {
			// TODO NLS
	...
	protected void encodeCostBlock(TcoObject root, List<? extends DbCodeType> codes, final String codeUndefined, final String title) throws IOException {
		...
		List totalCosts = calculator.getCostBlock(root); // .getDepreciationCostBlock(root,
	...
	private void encodeCostBlockColumns(TcoObject root, Object code) throws IOException {
		...
		List factCosts = calculator.getCosts(root, Calculator.KIND_FC /*
		 * CalculatorDepreciation . DEPRECIATION_LINEAR_COMPOUND_INTEREST
		 */, code);
