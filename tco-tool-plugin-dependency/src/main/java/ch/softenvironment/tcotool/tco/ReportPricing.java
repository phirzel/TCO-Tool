...
class ReportPricing extends ReportDependency {
     ...
     */
    public static ReportTool createBlock(ModelUtility utility, TcoObject root, long maxUsage, ClassLoader loader, boolean allServices, double multitude)
        throws Exception {
        ReportPricing tool = new ReportPricing(utility, ResourceManager.getResource(SEPlugin.class, "MniReportPricing_text", loader));
        tool.loader = loader;
    ...
    protected void encodeCostBlock(TcoObject object, List<? extends DbCodeType> codes, final String codeUndefined, String costTypeTitle) throws IOException {
        ...
        if (allServices || (!allServices && (depth > 0))) {
            ...
            Object[] tokens = {Double.valueOf(tmpFactor)};
            nativeContent("<font color='#FF0080'><bold>"
                + NlsUtils.formatMessage(ResourceManager.getResource(ReportPricing.class, "CICostFactor", loader), tokens) + " = " + af.format(sum)
                + "</bold></font>");
            getCsvWriter().cell(NlsUtils.formatMessage(ResourceManager.getResource(ReportPricing.class, "CICostFactor", loader), tokens));
            getCsvWriter().cell(getCsvWriter().encodeNumber(Double.valueOf(sum)));
            breakLine();
            encodeCostUnit("");
        }
    }
