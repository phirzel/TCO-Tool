...
class ReportDependency extends ReportTool {
     ...
     */
    public static ReportTool createBlock(ModelUtility utility, TcoObject root, long maxUsage, ClassLoader loader, boolean allServices) throws Exception {
        ReportDependency tool = new ReportDependency(utility, ResourceManager.getResource(SEPlugin.class,
            allServices ? "MniReportTcoDepenendencyDetailsAll_text" : "MniReportTcoDepenendencyDetails_text", loader));
        tool.loader = loader;
        ...
        if (allServices || (!allServices && (depth > 0))) {
            ...
            for (int col = 0; col < depth; col++) {
                tableHeader(ResourceManager.getResourceAsNonLabeled(DependencyDetailView.class, "PnlSupplier_text") + " " + (col + 1) + "."
                    + ResourceManager.getResource(ReportDependency.class, "CILevelIndirect", loader));
            }
            endTableRow();
    ...
    protected double encodeDependency(ModelUtility util, TcoObject object, int depth, int currentDepth, String color) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            TcoObject supplier = (TcoObject) util.findObject(dep.getSupplierId(), (TcoObject) util.getRoot());

            // ch.softenvironment.jomm.serialize.AttributeList attrs =
            // getAlignRight();
            startTableRow();
            String s = "--[" + MathUtils.fix(dep.getDistribution().doubleValue(), 1) + "%]--> ";
     ...
     */
    @Override
    protected void encodeCodes(DbObjectServer server, TcoObject root) throws IOException {
    ...
    private double calcDependency(ModelUtility util, Dependency dependency, int tcoYear) {
        TcoObject supplier = (TcoObject) util.findObject(dependency.getSupplierId(), (TcoObject) util.getRoot());
        // FactCost
        ...
        Iterator<Dependency> it = supplier.getSupplierId().iterator();
        while (it.hasNext()) {
            Dependency subDependency = it.next();
            // add indirect sub-dependencies recursively
            sum = sum + calcDependency(util, subDependency, tcoYear);
        }
        // dependency's percentage
        return sum / 100.0 * dependency.getDistribution().doubleValue();
    }
}